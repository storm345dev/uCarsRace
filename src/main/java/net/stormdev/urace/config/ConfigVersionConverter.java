...
import net.stormdev.urace.utils.IdMaterialConverter;
...
public class ConfigVersionConverter {
	public static FileConfiguration convert(FileConfiguration config, double target){
		uCarsRace.plugin.getLogger().info("Converting config to new format...");
		double td = target*10; //Target is in format n.n
		int t = (int)td;
		uCarsRace.plugin.getLogger().info("Config target: "+target);
		switch(t){
		case 11:{ //1.1 
			fromV1ToV2(config); 
		...
		default: uCarsRace.plugin.getLogger().info("No destination config version found for: "+target);
		}
		return config;
	}
	...
	public static List<String> convertItemsToNewFormat(String[] rawIds){
		...
		for (String raw : rawIds) {
			...
			uCarsRace.plugin.getLogger().info("Invalid config value: "+raw+", skipping...");
		}
	    return newIds;
	}
	public static List<String> convertSpeedModsToNewFormat(String[] rawIds){
		...
		for (String raw : rawIds) {
			...
			uCarsRace.plugin.getLogger().info("Invalid config speedmod: "+raw+", skipping...");
		}
	    return newIds;
	}
}
