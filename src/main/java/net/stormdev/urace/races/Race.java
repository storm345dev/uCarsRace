...
import net.stormdev.urace.players.*;
...
public class Race {
	...
	public Race(RaceTrack track, String trackName, RaceType type) {
		...
		this.tickrate = uCarsRace.config.getLong("general.raceTickrate");
		this.scorerate = (long) ((this.tickrate * 2) + (this.tickrate / 0.5));
		this.board = uCarsRace.plugin.getServer().getScoreboardManager()
				.getNewScoreboard();
		...
		this.timeLimitS = ((uCarsRace.config
		...
		for (User user : users) {
			try {
				...
				if (!forceRemoveUser(user)) {
					uCarsRace.logger.info("getUser() failed to remove user");
				}
			}
		}
		return null;
	...
	public void playerOut(User user) {
		...
		uCarsRace.plugin.hotBarManager.clearHotBar(user.getPlayerName());
		Player player = null;
		try {
			...
			if (!forceRemoveUser(user)
				...
				uCarsRace.logger.info("race.playerOut failed to remove user");
			}
			return;
		}
		if(player != null){
	...
	public void leave(User user, boolean quit) {
		...
		if (quit) {
			if (!forceRemoveUser(user)) {
				uCarsRace.logger.info("race.quit failed to remove user");
			}
			if (type != RaceType.TIME_TRIAL) {
				if (users.size() < 2) {
					for (User u : getUsersIn()) {
						String msg = uCarsRace.msgs.get("race.end.soon");
						try {
							u.getPlayer().sendMessage(
									uCarsRace.colors.getInfo() + msg);
						} catch (PlayerQuitException e) {
		...
		if (quit) {
			if (player != null) {
				...
				try {
					player.teleport(this.track.getExit(uCarsRace.plugin.getServer()));
					Bukkit.getScheduler().runTaskLater(uCarsRace.plugin, new BukkitRunnable(){
				...
				player.setScoreboard(uCarsRace.plugin.getServer()
	...
	public void start() {
		...
		this.task = uCarsRace.plugin.getServer().getScheduler()
				.runTaskAsynchronously(uCarsRace.plugin, new Runnable() {
							...
							if (tps < 19.9) {								
								if (tps < 13) {
									if(strikes < 5){
										...
										broadcast(uCarsRace.colors.getError()+"[Error] Race cancelled to compensate for "
									...
									uCarsRace.logger.info("[INFO] Current system TPS (From independent service): "+tps);
									uCarsRace.logger
		...
		this.scoreCalcs = uCarsRace.plugin.getServer().getScheduler()
				.runTaskTimer(uCarsRace.plugin, new Runnable() {
		...
		try {
			...
			e.printStackTrace();
			uCarsRace.logger.log("Error starting race!", Level.SEVERE);
			end();
		}
		return;
	}
	...
	public void end() {
		...
		try {
			...
			uCarsRace.logger.info("[IMPORTANT] Failed to process trailing end of race!");
			e.printStackTrace();
			//Race voided
		}
		try {
			clear();
			uCarsRace.plugin.raceScheduler.removeRace(this);
			uCarsRace.plugin.raceScheduler.recalculateQueues();
		} catch (Exception e) {
			uCarsRace.logger.info("[IMPORTANT] Failed to remove race");
			e.printStackTrace();
			//Race Voided
		}
		System.gc();
	...
	public void finish(User user) {
		...
		if (!forceRemoveUser(user)) {
			uCarsRace.logger.info("race.finish failed to remove user");
		}
	...
	public synchronized User updateUser(Player player) {
		...
		for (User u : getUsers()) {
			if (u.getPlayerName().equals(playerName)) {
				if (!forceRemoveUser(u)) {
					uCarsRace.logger.info("updateUser() failed to remove user");
				}
				u.setPlayer(player);
				users.add(u);
				return u;
	...
	public CheckpointCheck playerAtCheckpoint(Integer[] checks, Player p,
		...
		for (Integer key : checks) {
			try {
				...
				// better
				// performance
				p.removeMetadata("checkpoint.distance", uCarsRace.plugin);
				p.setMetadata("checkpoint.distance", new StatValue(dist,
						uCarsRace.plugin));
				if (dist < uCarsRace.plugin.checkpointRadiusSquared) {
					at = true;
					checkpoint = key;
