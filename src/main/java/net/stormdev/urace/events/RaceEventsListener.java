...
import net.stormdev.urace.races.RaceExecutor;
import net.stormdev.urace.races.uRaceFinishEvent;
...
public class RaceEventsListener implements Listener {
	...
	public RaceEventsListener(uCarsRace plugin){
		this.plugin = plugin;
	...
	void exploder(EntityExplodeEvent event) { //Stop bombs blowing up terrain
		if (!uCarsRace.config.getBoolean("mariokart.enable")) {
			return;
		}
	...
	void postRespawn(PlayerRespawnEvent event) { //Handle post respawns in races
		...
		Minecart cart = (Minecart) loc.getWorld().spawnEntity(loc,
				EntityType.MINECART);
		cart.setMetadata("kart.racing", new StatValue(null, uCarsRace.plugin));
		...
		uCarsRace.plugin.raceScheduler.updateRace(race);
		return;
	}
	...
	@EventHandler
	void raceFinish(uRaceFinishEvent event) { //Handle rewards after players finish a race
		Player player = event.getPlayer();
		uCarsRace.plugin.hotBarManager.clearHotBar(player.getName());
		if (!uCarsRace.config.getBoolean("general.race.rewards.enable")) {
			return;
		}
		...
		switch (pos) {
		case 1: {
			reward = uCarsRace.config.getDouble("general.race.rewards.win");
			break;
		}
		case 2: {
			reward = uCarsRace.config.getDouble("general.race.rewards.second");
			break;
		}
		case 3: {
			reward = uCarsRace.config.getDouble("general.race.rewards.third");
			break;
		}
		default:
			return;
		...
		if (!uCarsRace.vault || uCarsRace.economy == null) {
			plugin.setupEconomy(); // Economy plugin loaded after MarioKart
			if (!uCarsRace.vault || uCarsRace.economy == null) { // No Economy plugin
														// installed
				return;
			}
		}
		EconomyResponse r = uCarsRace.economy
				.depositPlayer(player.getName(), reward);
		double b = r.balance;
		String currency = uCarsRace.config
				.getString("general.race.rewards.currency");
		String msg = uCarsRace.msgs.get("race.end.rewards");
	...
	void pvp(EntityDamageEvent event) { //Stop PVP in races
		if (event.getEntity() instanceof Player
				&& uCarsRace.plugin.raceMethods.inAGame(
